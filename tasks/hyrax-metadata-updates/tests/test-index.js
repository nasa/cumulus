...
test('Test return prod OPeNDAP host when no environment value supplied', async (t) => {
  delete process.env.CMR_ENVIRONMENT;
  const actual = await generateAddress();
...
test('Test granule ur extraction from UMM-G', async (t) => {
  ...
  const actual = await getGranuleUr(metadata, true);
...
test('Test granule ur extraction from ECHO10', async (t) => {
  ...
  const actual = await getGranuleUr(metadata, false);
...
test('Test adding OPeNDAP URL to UMM-G file', async (t) => {
  ...
  const actual = await addHyraxUrl(metadata, true, 'https://opendap.earthdata.nasa.gov/providers/GES_DISC/collections/GLDAS%20Catchment%20Land%20Surface%20Model%20L4%20daily%200.25%20x%200.25%20degree%20V2.0%20(GLDAS_CLSM025_D)%20at%20GES%20DISC/granules/GLDAS_CLSM025_D.2.0:GLDAS_CLSM025_D.A20141230.020.nc4');
