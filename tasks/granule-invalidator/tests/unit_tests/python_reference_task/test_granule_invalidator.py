...
from .. import granule_invalidator, invalidations


def test_run_invalidation_science_date_invalidation() -> None:
    with patch('src.invalidations.science_date') as science_date_invalidation:
...
def test_run_invalidation_ingest_date_invalidation() -> None:
    with patch('src.invalidations.ingest_date') as ingest_date_invalidation:
...
def test_run_invalidation_cross_collection_invalidation() -> None:
    with patch('src.invalidations.cross_collection') as cross_collection_invalidation:
...
def test_identify_granules_older_than() -> None:
    ...
    with patch('src.invalidations.datetime', wraps=datetime.datetime) as datetime_mock:
...
def test_ingest_date_invalidation() -> None:
    now = '2021-01-01T00:14:30Z'
    # 1609460040000 = '2021-01-01T00:14:00Z', 1609459980000 = '2021-01-01T00:13:00Z'
...
def test_cross_collection_invalidation() -> None:
    ...
    with patch('src.granule_invalidator.fetch_all_granules') as fetch_all_granules:
