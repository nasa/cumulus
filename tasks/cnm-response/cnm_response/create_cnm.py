...
# Specify version
# use s3 or https 'uri' with distribution url parameter
class CnmGenerator:
...
def _get_response(exception: str | None) -> dict:
    if not exception or exception in {"None", '"None"'}:
        return {
            "status": "SUCCESS",
        }
...
def _get_checksum(file: dict) -> dict:
    checksum = {}
