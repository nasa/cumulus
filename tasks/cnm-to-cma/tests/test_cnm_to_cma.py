...
class TestCNMToCMA:
    ...
    def test_mapper(self):
        with open("tests/resources/cumulus_sns_v1.0_notification.json") as f:
            data = json.load(f)
            config = {
                "collection": {
            ...
            assert len(granule_files) == 2  # noqa: PLR2004
            ...
            assert granule_files[0].path == "prod_20170926T11:30:36"
            ...
            assert granule_files[1].path == "prod_20170926T11:30:36"

    def test_granule_extraction(self):
        with open("tests/resources/cumulus_sns_v1.0_JA1_GPN_E.json") as f:
            data = json.load(f)
            config = {
                "collection": {
                    "name": "test_collection",
            ...
            assert len(granule.files) == 2  # noqa: PLR2004
            assert granule.granuleId == (
                "JA1_GPN_2PeP001_002_20020115_060706_20020115_070316"
            )

    def test_mapper_with_wrong_formatted_json(self):
        with open(
            "tests/resources/cumulus_sns_v1.0_notification_incorrect_formatted.json"
        ) as f:
            data = json.load(f)
    ...
    def test_build_granule_file(self):
        ...
        """
        with open(
            "tests/resources/cumulus_sns_v1.0_notification_multi_protocols.json"
        ) as f:
            cnm = json.load(f)
            ...
            assert len(cnm_files) == 4  # noqa: PLR2004
            granule_files: list[models_granule.File] = create_granule_files(cnm_files)
            assert len(granule_files) == 4  # noqa: PLR2004
            # verify every item
            assert granule_files[0].name == "production_file.nc"
            assert granule_files[0].filename == "production_file.nc"
            assert granule_files[0].type == "data"
            ...
            assert granule_files[1].type == "browse"
            assert granule_files[1].source_bucket is None
            ...
            assert granule_files[2].source_bucket is None
            ...
            assert granule_files[3].source_bucket is None
            assert granule_files[3].path == "sftp_path"

    def test_get_cnm_input_files(self):
        with open("tests/resources/cumulus_sns_v1.1_filegroups_multiple.json") as f:
            cnm = json.load(f)
            cnm_model = models_cnm.CloudNotificationMessageCnm12.model_validate(cnm)
            cnm_files = get_cnm_input_files(cnm_model.root.product)
