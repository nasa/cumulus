...
class TestCNMToCMA:
    ...
    def test_mapper(self):
        with open(
            "tests/resources/cumulus_sns_v1.0_notification.json"
        ) as f:
            data = json.load(f)
            config = {
                "collection": {
    ...
    def test_granule_extraction(self):
        with (open(
            "tests/resources/cumulus_sns_v1.0_JA1_GPN_E.json"
        ) as f):
            data = json.load(f)
            config = {
                "collection": {
                    ...
                    "version": "001",
                    "granuleIdExtraction":
                        "^(JA1_GPN_2PeP([0-9]{3})_([0-9]{3})_([0-9]{8})_([0-9]{6})"
    ...
    def test_get_cnm_input_files(self):
        with open(
            "tests/resources/cumulus_sns_v1.1_filegroups_multiple.json"
        ) as f:
            cnm = json.load(f)
            cnm_model = models_cnm.CloudNotificationMessageCnm12.model_validate(cnm)
            cnm_files = get_cnm_input_files(cnm_model.root.product)
