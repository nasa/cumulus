...
exports.uploadFile = async (client, destinationDir, fileName, fileDataStream) => {
  ...
  //return syncPut('/tmp/README.md', 'PDRD/README.md');
  // log.info('Calling client put');
  // client.status((err, status) => {
  //   if (err) log.error(err);
  //   if (status) log.info(status);
  // });
  // client.put('/tmp/README.md', 'test.txt', (err) => log.error(err));
};
...
/**
 * Get a readable stream to a file on a SIPS server
 * @param {Client} client The client connected to the SIPS server
 * @param {*} filePath The path to the file to retrieve
 * @return An ReadableStream to the file
 */
exports.getFileStream = async (client, filePath) => {
  const syncGet = promisify(client.get).bind(client);
  return await syncGet(filePath);
};
