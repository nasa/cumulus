...
/*
 This is the function written as if the API needs to be called. For now it seems as though it doesn't.
   exports.getSfnExecutionByName = async (stateMachineArn, executionName) => {
   const sfn = exports.sfn();
   let nextToken;
   do {
   const executionData = await sfn.listExecutions({
   ...
   log.info('EXECUTIONS (', stateMachineArn, executionName, ')', JSON.stringify(executionData));
   ...
   nextToken = executionData.nextToken;
   } while (nextToken);
...
exports.getCurrentSfnTask = async (stateMachineArn, executionName) => {
  ...
  log.info('HISTORY', JSON.stringify(executionHistory));
