...
exports.downloadS3Keys = (bucket, keys, dir, s3opts = {}) => {
  const s3 = exports.s3();
  let i = 0;
  const n = keys.length;
  log.info(`Starting download of ${n} keys to ${dir}`);
  const promiseDownload = (key) => {
    const filename = path.join(dir, path.basename(key));
    const file = fs.createWriteStream(filename);
    const opts = Object.assign({ Bucket: bucket, Key: key }, s3opts);
    return new Promise((resolve, reject) => {
      s3.getObject(opts)
        .createReadStream()
        .pipe(file)
        .on('finish', () => {
          log.info(`Progress: [${i++} of ${n}] s3://${bucket}/${key} -> ${filename}`);
          return resolve(key);
        })
        .on('error', reject);
    });
  ...
  return Promise.all(keys.map(limitedDownload));
};
...
exports.syncUrl = (url, bucket, destKey) =>
  concurrency.promiseUrl(url)
    .then((response) =>
