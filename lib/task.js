...
module.exports = class Task {
  constructor(source, config, state, message, context) {
    this.config = config;
    this.source = source;
  ...
  exclusive(key, timeout, fn) {
    ...
    const table = this.message.resources.tables.locks;
  ...
  static async invoke(task) {
    try {
      ...
      if (task.config && task.config.connections) {
        ...
        const providerConfig = task.message.provider;
        const max = providerConfig.config.global_connection_limit;
        const key = providerConfig.id;
        const table = task.message.resources.tables.connections;
  ...
  static async handle(message = {}, context = {}, callback = (() => null)) {
    ...
    try {
      ...
      const source = messageSource.forMessage(message, context);
      const [state, messageData] = await Promise.all([
        source.loadState(this),
        source.loadMessageData()]);

      const configTemplate = await source.loadConfigTemplate();

      const config = FieldPattern.formatAll(configTemplate, messageData);

      log.info('   Config:', JSON.stringify(config));
      log.info('   Loaded Message:', JSON.stringify(messageData));

      // Create and invoke the task
      const task = new this(source, config, state, messageData, context, log);
