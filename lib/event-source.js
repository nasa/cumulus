...
const EventSource = exports.EventSource = class {
  ...
  loadConfigTemplate() {
    ...
    if (task) {
      return this.eventData.workflow_config[task];
    }
...
class InlineEventSource extends EventSource {
  ...
  lambdaCallback(handler, callback, error, data) {
    ...
    super.lambdaCallback(handler, callback, error, outputData);
  }
}
