...
import {
  addCollections,
} from '@cumulus/integration-tests';
import apiTestUtils from '@cumulus/integration-tests/api/api';
// import {
...
// const createTestSuffix = (prefix: string) => `_test-${prefix}`;
// const apiTestDir = '../packages/api/tests';
const collectionsDir = 'data/collections/s3_MOD09GQ_006_full_ingest/';

const createTimestampedTestId = (stackName: string, testName: string) =>
  `${stackName}-${testName}-${Date.now()}`;

// function* createGranuleIdGenerator(total: number) {
//   for (let i = 0; i < total; i += 1) {
//     yield `${cryptoRandomString({ length: 7 })}.${cryptoRandomString({ length: 20 })}.hdf`;
//   }
// }

const addCollection = async (stackName: string, bucket: string, testSuffix: string) => {
...
const addProvider = async (stackName: string, bucket: string, testSuffix: string) => {
...
// const uploadDBGranules = async (batchSize: number, testSuffix: string) => {
