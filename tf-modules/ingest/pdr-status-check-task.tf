resource "aws_lambda_function" "pdr_status_check_task" {
  ...
  memory_size      = lookup(var.lambda_memory_sizes, "pdr_status_check_task_memory_size", 1024)
...
resource "aws_cloudwatch_log_group" "pdr_status_check_task" {
  name              = "/aws/lambda/${aws_lambda_function.pdr_status_check_task.function_name}"
  retention_in_days = lookup(var.cloudwatch_log_retention_periods, "ingest_pdr_status_check_task_log_retention", var.default_log_retention_days)
  tags              = var.tags
}
