resource "aws_secretsmanager_secret" "message_template_cmr_password" {
  ...
  description = "CMR password for use in Cumulus's message template in the ${var.prefix} deployment"
...
resource "aws_secretsmanager_secret" "message_template_launchpad_passphrase" {
  ...
  description = "Launchpad passphrase for use in Cumulus's message template in the ${var.prefix} deployment"
  tags        = local.default_tags
}
