...
resource "aws_cloudwatch_event_target" "cw_sf_event_to_db_records" {
  rule = aws_cloudwatch_event_rule.state_machine_execution_rule.name
  arn  = var.workflow_config.cw_sf_event_to_db_records_lambda_function_arn
}

resource "aws_lambda_permission" "cw_sf_event_to_db_records" {
  action        = "lambda:InvokeFunction"
  function_name = var.workflow_config.cw_sf_event_to_db_records_lambda_function_arn
  principal     = "events.amazonaws.com"
  source_arn    = aws_cloudwatch_event_rule.state_machine_execution_rule.arn
}
