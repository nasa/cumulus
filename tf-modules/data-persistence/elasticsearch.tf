...
resource "aws_security_group" "es_vpc" {
  count  = local.deploy_inside_vpc && var.elasticsearch_security_group_id == null ? 1 : 0
...
resource "aws_elasticsearch_domain" "es_vpc" {
  ...
  vpc_options {
    ...
    security_group_ids = var.elasticsearch_security_group_id == null ? [aws_security_group.es_vpc[0].id] : [var.elasticsearch_security_group_id]
  }
