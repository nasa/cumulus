resource "aws_lambda_function" "archive_records" {
  ...
  filename         = "${path.module}/../../packages/api/dist/archive-records/lambda.zip"
  source_code_hash = filebase64sha256("${path.module}/../../packages/api/dist/archive-records/lambda.zip")
