...
resource "aws_lambda_function" "create_reconciliation_report" {
  ...
  environment {
    variables = {
      ...
      ES_SCROLL                        = lookup(var.api_lambda_vars, "create_reconciliation_report_es_scroll", "6m")
      ES_SCROLL_SIZE                   = lookup(var.api_lambda_vars, "create_reconciliation_report_es_scroll_size", 1000)
