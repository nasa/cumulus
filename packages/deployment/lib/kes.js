...
class UpdatedKes extends Kes {
  ...
  async getRetainedLambdaAliasMetadata() {
    ...
    lambdaNames.forEach((lambdaName) => {
      ...
      oldLambdaRefs = oldLambdaRefs.concat(oldAliasMetadataObjects);
    });
    return oldLambdaRefs;
  }


  /**
   * Parses a passed in alias descriptiong field for a version string,
   * (e.g. `Cumuluas Autogenerated Alias |version`)
   *
