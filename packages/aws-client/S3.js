...
// Each part of a multi-part copy needs to specify a byte range to be copied. This byte range has a
// starting byte and an ending byte (inclusive) that makes up the part. All parts must be larger
// than 5 MB, except that the last part may be smaller than 5 MB. Parts cannot be larger than 5 GB.
//
// This function takes a file size and an optional maxSize. It returns an array of objects, each
// containing a `start` and an `end` value. These will make up the ranges of the multi-part copy.
//
...
// This performs an S3 `uploadPartCopy` call. That response includes an `ETag`, which is needed when
// `completeMultipartUpload` is called. `completeMultipartUpload` also requires the `PartNumber`for
// each part. That value is not included in the `uploadPartCopy` response so, for simplicity's sake,
// we're adding that value to the response here.
...
// Given a list of chunks (as generated by `createMultipartChunks`), create the list of params that
// will be passed to each invocation of the `uploadPartCopy` function.
...
// Generate the params that will be passed to the `completeMultipartUpload` function
