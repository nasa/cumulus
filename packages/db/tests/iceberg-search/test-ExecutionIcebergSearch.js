...
const { ExecutionIcebergSearch } = require('../../dist/iceberg-search/ExecutionIcebergSearch');
const {
...
test.serial('ExecutionIcebergSearch returns correct response for basic query', async (t) => {
...
test.serial('ExecutionIcebergSearch supports page and limit params', async (t) => {
  const { connection } = t.context;
  ...
  let dbSearch = new ExecutionIcebergSearch({ queryStringParameters }, connection);
  ...
  dbSearch = new ExecutionIcebergSearch({ queryStringParameters }, connection);
...
test.serial('ExecutionIcebergSearch supports infix search', async (t) => {
  const { connection } = t.context;
  const queryStringParameters = {
    limit: 50,
    infix: 'fake',
...
test.serial('ExecutionIcebergSearch supports prefix search', async (t) => {
  const { connection } = t.context;
  const queryStringParameters = {
    limit: 50,
    prefix: 'test',
...
test.serial('ExecutionIcebergSearch supports collectionId term search', async (t) => {
  const { connection } = t.context;
  const queryStringParameters = {
    limit: 50,
...
test.serial('ExecutionIcebergSearch supports asyncOperationId term search', async (t) => {
  const { connection } = t.context;
  const queryStringParameters = {
    limit: 50,
...
test.serial('ExecutionIcebergSearch supports term search for number field', async (t) => {
  const { connection } = t.context;
  const queryStringParameters = {
    limit: 50,
    duration: 100,
...
test.serial('ExecutionIcebergSearch supports term search for string field', async (t) => {
  const { connection } = t.context;
  let queryStringParameters = {
    limit: 50,
    status: 'completed',
...
test.serial('ExecutionIcebergSearch supports term search for nested error.Error', async (t) => {
  const { connection } = t.context;
  const queryStringParameters = {
    limit: 200,
...
test.serial('ExecutionIcebergSearch supports range search', async (t) => {
  const { connection } = t.context;
  let queryStringParameters = {
    limit: 50,
    duration__from: 100,
...
test.serial('ExecutionIcebergSearch non-existing fields are ignored', async (t) => {
  const { connection } = t.context;
  const queryStringParameters = {
    estimateTableRowCount: 'false',
    limit: 200,
...
test.serial('ExecutionIcebergSearch returns fields specified', async (t) => {
  const { connection } = t.context;
...
test.serial('ExecutionIcebergSearch supports search for multiple fields', async (t) => {
  const { connection } = t.context;
  const queryStringParameters = {
    id: 13,
...
test.serial('ExecutionIcebergSearch supports sorting', async (t) => {
  const { connection } = t.context;
  let queryStringParameters = {
    estimateTableRowCount: 'false',
    limit: 50,
  ...
  const dbSearch2 = new ExecutionIcebergSearch({ queryStringParameters }, connection);
  ...
  const dbSearch3 = new ExecutionIcebergSearch({ queryStringParameters }, connection);
...
test.serial('ExecutionIcebergSearch supports sorting by Error', async (t) => {
  const { connection } = t.context;
  let queryStringParameters = {
    limit: 50,
    sort_by: 'error.Error',
  };
  const dbSearch7 = new ExecutionIcebergSearch({ queryStringParameters }, connection);
  ...
  const dbSearch10 = new ExecutionIcebergSearch({ queryStringParameters }, connection);
...
test.serial('ExecutionIcebergSearch supports terms search', async (t) => {
  const { connection } = t.context;
...
test.serial('ExecutionIcebergSearch supports parentArn term search', async (t) => {
  const { connection } = t.context;
  let queryStringParameters = {
...
test.serial('ExecutionIcebergSearch supports error.Error terms search', async (t) => {
  const { connection } = t.context;
  let queryStringParameters = {
    limit: 50,
...
test.serial('ExecutionIcebergSearch supports search which checks existence of execution field', async (t) => {
  const { connection } = t.context;
  const queryStringParameters = {
    limit: 50,
...
test.serial('ExecutionIcebergSearch supports search which execution field does not match the given value', async (t) => {
  const { connection } = t.context;
  const queryStringParameters = {
    limit: 50,
...
test.serial('ExecutionIcebergSearch supports term search for timestamp', async (t) => {
  const { connection } = t.context;
  let queryStringParameters = {
    limit: 50,
...
test.serial('ExecutionIcebergSearch supports term search for date field', async (t) => {
  const { connection } = t.context;
  const queryStringParameters = {
    limit: 50,
...
test.serial('ExecutionIcebergSearch includeFullRecord', async (t) => {
  const { connection } = t.context;
  const queryStringParameters = {
    estimateTableRowCount: 'false',
    limit: 50,
...
test.serial('ExecutionIcebergSearch only returns count if countOnly is set to true', async (t) => {
  const { connection } = t.context;
  const queryStringParameters = {
    countOnly: 'true',
  };
...
test.serial('ExecutionIcebergSearch with archived: true pulls only archive granules', async (t) => {
  const { connection } = t.context;
  const queryStringParameters = {
    archived: 'true',
  };
...
test.serial('ExecutionIcebergSearch with archived: false pulls only non-archive granules', async (t) => {
  const { connection } = t.context;
  const queryStringParameters = {
    archived: 'false',
  };
...
test.serial('ExecutionIcebergSearch returns the correct record', async (t) => {
  const { connection } = t.context;
