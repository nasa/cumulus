...
test('FilePgModel.upsertById() updates the unique indentifiers for a file record', async (t) => {
  ...
  const [insertedGranuleId] = await filePgModel.create(knex, file);

  const updatedFileWithUpdatedUniqueKeys = {
    ...file,
    cumulus_id: insertedGranuleId,
    bucket: 'NewBucket',
    key: 'NewKey',
  };

  await filePgModel.upsertById(knex, updatedFileWithUpdatedUniqueKeys);

  t.like(
    await filePgModel.get(knex, {
      cumulus_id: insertedGranuleId,
    }),
    updatedFileWithUpdatedUniqueKeys
  );
});
...
test('FilePgModel.upsertById() fails if updating by ID and a unique identifer collision occurs', async (t) => {
  ...
  await t.throwsAsync(filePgModel.upsertById(knex, updatedFileWithUpdatedUniqueKeys));
});
