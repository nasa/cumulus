...
test('ProviderSearch supports range search', async (t) => {
  ...
  let queryStringParameters = {
    limit: 200,
    timestamp__from: `${t.context.providerSearchTmestamp + 1}`,
    timestamp__to: `${t.context.providerSearchTmestamp + 2}`,
  };
  let dbSearch = new ProviderSearch({ queryStringParameters });
  let response = await dbSearch.query(knex);
  t.is(response.meta.count, 50);
  t.is(response.results?.length, 50);
  t.true(response.results?.every(provider => provider.updatedAt >= t.context.providerSearchTmestamp + 1 && provider.updatedAt <= t.context.providerSearchTmestamp + 2));
});
...
test('ProviderSearch supports search for multiple fields', async (t) => {
  ...
  t.is(response.results?.length, 1);
  t.true(response.results[0].host === 'cumulus-sit' && response.results[0].id === 'testProvider82' && response.results[0].globalConnectionLimit === 0);
});
