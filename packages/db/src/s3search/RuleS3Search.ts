...
import { RuleRecordWithExternals, RuleSearch } from '../search/RuleSearch';
import { QueryEvent } from '../types/search';
import { prepareBindings } from './duckdbHelpers';

const log = new Logger({ sender: '@cumulus/db/ExecutionS3Search' });
  ...
  async query() {
    ...
    try {
      ...
      const apiRecords = await this.translatePostgresRecordsToApiRecords(
        pgRecords as unknown[] as RuleRecordWithExternals[]
      );
