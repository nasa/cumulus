 ...
 * @returns {Promise}
 */
const _deleteS3Files = async (files = []) => p_map_1.default(files, (file) => {
    S3_1.deleteS3Object(FileUtils.getBucket(file), FileUtils.getKey(file));
});
/**
 ...
 */
const deleteGranuleAndFiles = async ({ knex, dynamoGranule, pgGranule, filePgModel = new db_1.FilePgModel(), granulePgModel = new db_1.GranulePgModel(), granuleModelClient = new Granule(), }) => {
