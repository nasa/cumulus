...
test.serial('reingest a granule', async (t) => {
  ...
  const list = JSON.stringify([{ name: payload.meta.workflow_name, arn: 'arn:fake' }]);
  await Promise.all([
    aws.s3().putObject({ Bucket: process.env.system_bucket, Key: key, Body: 'test data' }).promise(),
