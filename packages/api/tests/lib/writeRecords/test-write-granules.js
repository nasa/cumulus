...
test.serial('writeGranulesFromMessage() publishes SNS message with event type Create if a granule is being created', async (t) => {
  const {
    cumulusMessage,
    granuleModel,
    knex,
  ...
  try {
    ...
    t.is(error.name, 'RecordDoesNotExist');
  }
...
test.serial('writeGranulesFromMessage() publishes SNS message with event type Update if a granule is being updated', async (t) => {
  const {
    cumulusMessage,
    granuleModel,
    knex,
  ...
  await t.context.granulePgModel.upsert(
    knex,
    postgresGranuleRecord
  );
  ...
  t.is(publishedMessage.record.granuleId, granule.granuleId);
