...
const { sendCreateTopicCommand, sendDeleteTopicCommand } = require('@cumulus/aws-client/SNS');
...
test.before(async (t) => {
  ...
  const { TopicArn } = await sendCreateTopicCommand({ Name: randomString() });
...
test.after.always(async (t) => {
  ...
  await sendDeleteTopicCommand({ TopicArn: t.context.granules_sns_topic_arn });
});
