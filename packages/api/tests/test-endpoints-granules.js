...
test('move a file and update metadata', async (t) => {
  ...
  await aws.s3().createBucket({ Bucket: buckets.private.name }).promise();
  const newGranule = fakeGranuleFactory();
  ...
  newGranule.files = [
    ...
    },
    {
      ...
      filepath: `${process.env.stackName}/granules_ingested/${newGranule.granuleId}.cmr.xml`,
      filename: `s3://${buckets.private.name}/${process.env.stackName}/granules_ingested/${newGranule.granuleId}.cmr.xml`
    }
  ];
