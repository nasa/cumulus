...
test.serial('writeGranules() does not overwrite original failure if file write fails', async (t) => {
  const {
    cumulusMessage,
    knex,
    collectionCumulusId,
  ...
  cumulusMessage.payload.granules[0].status = 'failed';
  cumulusMessage.exception = {
    Error: 'test error',
    Cause: new Error('test failure').toString(),
  };
  ...
  const pgGranule = await t.context.granulePgModel.get(knex, { granule_id: granuleId });
  t.is(pgGranule.error.Error, 'test error');
  t.is(pgGranule.error.Cause, 'Error: test failure');
});
