...
const pgPayloadsEmpty = (entry, t) => {
  return !Boolean(entry.final_payload) && !Boolean(entry.orginal_payload)
}
const esPayloadsEmpty = (entry, t) => {
  return !Boolean(entry.finalPayload) && !Boolean(entry.orginalPayload)
}
test('handler() handles complete expiration', async (t) => {
  ...
  await handler();
  const model = new ExecutionPgModel()
  let massagedPgExecutions = await Promise.all(t.context.execution_cumulus_ids.map(async (cumulus_id) => await model.get(t.context.knex, {cumulus_id})));
  ...
  await handler();
  massagedPgExecutions = await Promise.all(t.context.execution_cumulus_ids.map(async (cumulus_id) => await model.get(t.context.knex, {cumulus_id})));
