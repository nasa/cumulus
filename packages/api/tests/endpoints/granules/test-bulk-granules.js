...
test.serial('POST /granules/bulk returns 400 when no granules are provided', async (t) => {
  const { asyncOperationStartStub } = t.context;
  const expectedQueueName = 'backgroundProcessing';
  const expectedWorkflowName = 'HelloWorldWorkflow';
...
test.serial('POST /granules/bulk returns 400 when no workflowName and no granules is provided', async (t) => {
  const { asyncOperationStartStub } = t.context;
  const expectedQueueName = 'backgroundProcessing';
