...
test('getByHost() returns the provider if it exists', async (t) => {
  const { providersModel } = t.context;
  ...
  const fetchedProvider = await providersModel.getByHost(provider.host);

  const expectedProvider = {
    ...provider,
    createdAt: fetchedProvider.createdAt,
    updatedAt: fetchedProvider.updatedAt,
  };

  t.deepEqual(fetchedProvider, expectedProvider);
});

test('getByHost() returns undefined if the provider was not found', async (t) => {
  const { providersModel } = t.context;

  const fetchedProvider = await providersModel.getByHost(randomString());

  t.is(fetchedProvider, undefined);
});

test('getByHost() throws an exception if multiple providers are found with the same host', async (t) => {
  const { providersModel } = t.context;

  const host = randomString();

  await providersModel.create(fakeProviderFactory({ host }));
  await providersModel.create(fakeProviderFactory({ host }));

  await t.throwsAsync(providersModel.getByHost(host));
});
