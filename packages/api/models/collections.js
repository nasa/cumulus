...
function checkRegex(regex, sampleFileName) {
  ...
  if (!match) throw new InvalidDataError(`regex cannot validate ${sampleFileName}`);
}
...
class Collection extends Manager {
  static recordIsValid(item, schema = null) {
    ...
    if (!match) {
      throw new InvalidDataError('granuleIdExtraction regex returns null when applied to sampleFileName');
    }
