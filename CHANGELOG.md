...
- **MAAP-Project** - Add an `http-basicauth` protocol option to `@cumulus/ingest`. When ingesting granules, users can specify a provider protocol `http-basicauth` and username and password via `INGEST_USERNAME` and `INGEST_PASSWORD` environment variables. These values, along with a configurable number of redirects, enable syncing files behind login.
