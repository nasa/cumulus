...
resource "aws_ecs_service" "iceberg_api" {
  ...
  network_configuration {
    ...
    security_groups  = [aws_security_group.iceberg_ecs_task_sg.id, local.rds_security_group]
...
resource "aws_appautoscaling_target" "iceberg_api" {
  ...
  resource_id        = "service/${module.cumulus.ecs_cluster_name}/${aws_ecs_service.iceberg_api.name}"
