module "discover_granules_azure_workflow" {
  source = "../../tf-modules/workflow"

  prefix          = var.prefix
  name            = "DiscoverGranulesQueueToAzure"
  workflow_config = module.cumulus.workflow_config
  system_bucket   = var.system_bucket
  tags            = local.tags

  state_machine_definition = templatefile(
    "${path.module}/discover_granules_azure_workflow.asl.json",
    {
      ingest_granule_workflow_name: module.azure_processing_workflow.name,
