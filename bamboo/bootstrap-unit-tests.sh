...
$docker_command "npm install --error --no-progress -g nyc; cd $UNIT_TEST_BUILD_DIR ; npm install --error  --no-progress; npm run bootstrap-no-build-quiet; npm run bootstrap-no-build-quiet"
