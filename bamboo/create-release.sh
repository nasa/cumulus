...
curl -X POST -H "Authorization: token $GITHUB_TOKEN" --data-binary "@terraform-aws-tea-distribution.zip" -H "Content-type: application/octet-stream" $RELEASE_URL/assets?name=terraform-aws-tea-distribution.zip
