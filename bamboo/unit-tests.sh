...
then 
docker exec -i ${container_id}-build_env-1 /bin/bash -c "cd $UNIT_TEST_BUILD_DIR && aws s3 sync unit-logs/@cumulus/ s3://test-error-outputs/$(git rev-parse --abbrev-ref HEAD)/$(date +%DT%H.%M.%S)/"
fi
