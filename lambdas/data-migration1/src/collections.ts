...
export const migrateCollectionRecord = async (
  ...
  const updatedRecord: CollectionRecord = {
    ...
    duplicate_handling: dynamoRecord.duplicateHandling,
    granule_id_validation_regex: dynamoRecord.granuleId,
    granule_id_extraction_regex: dynamoRecord.granuleIdExtraction,
    // have to stringify on an array of values
    files: JSON.stringify(dynamoRecord.files),
    report_to_ems: dynamoRecord.reportToEms,
    sample_file_name: dynamoRecord.sampleFileName,
    ignore_files_config_for_discovery: dynamoRecord.ignoreFilesConfigForDiscovery,
