...
export const up = async (knex: Knex): Promise<void> =>
  knex.schema.createTable('collections', (table) => {
    ...
    table.boolean('reportToEms').comment('Flag to set if this granule should be reported to EMS');
    table.boolean('ignoreFilesConfigForDiscovery').comment(
      'When true, ignore the collection files config list for determining which files to ingest for a granule'
