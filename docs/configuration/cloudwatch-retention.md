...
2. `retention-in-days`: the number of days you'd like to retain the logs in the specified log group for. There is a list of possible values available in the [aws logs documentation](https://docs.aws.amazon.com/cli/latest/reference/logs/put-retention-policy.html). In order to specifically configure this value in terraform, add in <module>_<cloudwatch_log_group_name>_log_retention value (in days) to the cloudwatch_log_retention_groups map in tfvars.
