...
```
aws kinesis get-shard-iterator --shard-id shardId-111111111111 --shard-iterator-type LATEST --stream-name NOTIFICATION_STREAM_NAME_HERE
```
...
From the AWS services page select Simple Queue Service (SQS). Select your queue, and under the `Queue Actions` tab, you can choose View/Delete Messages.
Start polling for messages and you will see records that failed to process through the kinesis consumer workflow.  Note that these are only records that didn't get their workflows started successfully.  Workflow failures are another topic entirely.
