...
The two primary external limitations to throughput are copying assets from the data's source to S3 and creating granules in CMR. In general, copying from the data's source to S3 is slow with respect to typical data access speeds, and much slower than intra-S3 transfers. This a challenge with imagery data files that are often very large, and further compounded when there is a large variability among data file sizes in a group (e.g., some are megabytes in size and take seconds to transfer, and others are gigabytes in size and take tens of minutes to transfer). Additionally, some data source servers may have restrictions on how many simultaneous downloads are allowed per-IP.
