...
![Iceberg Architecture Diagram](../assets/iceberg_arch.png)
...
1. executions
2. granules
3. files (Postgres) to files_table (Iceberg) - note the name in Iceberg is different due to clashes with a reserved files concept in Iceberg.
4. all other tables
...
![Iceberg Bootstrap Process Diagram](../assets/iceberg_bootstrap.png)
...
See [Iceberg troubleshooting](../troubleshooting/troubleshoot_iceberg.md).
...
Both Iceberg and DuckDB are evolving and receive new features over time. On May 29, 2026 there was a major update for the Iceberg extension in DuckDB in release [1.5.3](https://duckdb.org/2026/05/29/new-iceberg-features) which adds several features we would have liked to use from the start and adds support for V3 format. We should continue to monitor and update our Iceberg implementation as improvements are made.
...
Going forward we should implement a solution that does not result in a time period of missing data. The solution we would recommend is creating migrations and use a [newly released feature for DuckDB](https://duckdb.org/2026/05/29/new-iceberg-features?#alter-table-support) to update the schema to match the migration made to Postgres.
