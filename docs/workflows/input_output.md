...
* `input`: By default, the incoming payload is the payload output from the previous task, or it can be configured to be any portion of the payload using the `config` key.
