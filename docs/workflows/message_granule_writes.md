...
- Message granule must match the [API Granule schema](https://github.com/nasa/cumulus/blob/master/packages/api/lib/schemas.js), except `producerGranuleId` is optional. If `producerGranuleId` is set to `granuleId` if it's absent in the message granule.
